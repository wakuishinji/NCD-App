...
5. **コミット & Push**: 作業を完了したら `git add` → `git commit` → `git push` を実行。Push が成功するまでは完了報告しない。  
   ...
   - **Push 直後に `wrangler deploy --env staging`（または和久井さん指定の env デプロイ）を必ず実行し、最新差分をステージングへ反映させてから完了報告する。**  
   - **Workers デプロイ完了後、Cloudflare Pages へのデプロイ（`npm run deploy:pages:staging` 等、指定コマンド）も毎回実行し、`.pages.dev` ステージングへ最新 UI を揃える。**  
